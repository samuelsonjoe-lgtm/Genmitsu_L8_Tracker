...
- **Designs** - session-only offline SVG generators for freestanding QR/sign stands, hanging signs, dice trays, divider trays, and open-top finger-jointed boxes with an optional loose flat lid. The box generator accepts inside or outside-body dimensions, measured material thickness, nonnegative joint clearance, and a preferred finger-width target, then previews the exact validated SVG used for download. Joint clearance adjusts assembly fit but does not replace material-specific kerf compensation in LightBurn. Test every fit on scrap before production. Designs are not saved and are excluded from JSON backups.
